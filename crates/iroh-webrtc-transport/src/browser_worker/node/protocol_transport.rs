...
#[derive(Debug, Clone)]
pub(super) struct WorkerProtocolTransportHook {
    inner: Weak<Mutex<BrowserWorkerNodeInner>>,
}

impl EndpointHooks for WorkerProtocolTransportHook {
    async fn before_connect<'a>(
        ...
        let (registered, transport_intent, prepare_tx) = {
            let inner = inner.lock().expect("browser worker node mutex poisoned");
            (
        ...
        let request = WorkerProtocolTransportPrepareRequest {
        ...
        match result.await {
            ...
            Ok(Err(err)) => {
                tracing::debug!(
                    target: "iroh_webrtc_transport::browser_worker::protocol_transport",
                    %err,
                    "failed to prepare worker protocol WebRTC transport"
                );
...
impl BrowserWorkerNode {
    pub(super) fn protocol_transport_hook(&self) -> WorkerProtocolTransportHook {
        WorkerProtocolTransportHook {
            inner: Arc::downgrade(&self.inner),
        }
    }

    pub(in crate::browser_worker) fn add_protocol_transport_session_addr(
        &self,
        remote: EndpointId,
        dial_id: DialId,
    ) -> BrowserWorkerResult<()> {
    ...
    #[cfg(test)]
    pub(in crate::browser_worker) fn protocol_transport_endpoint_addr(
        &self,
        remote: EndpointId,
    ) -> Option<EndpointAddr> {
        let inner = self
...
fn reject_non_webrtc_protocol_connection() -> AfterHandshakeOutcome {
    AfterHandshakeOutcome::Reject {
        ...
        reason: b"worker protocol requires WebRTC custom transport".to_vec(),
    }
}
